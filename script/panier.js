...
export const addToCart = (elem) => {
    ...
    if ($(elem.currentTarget.previousElementSibling).val() != 0) {
        ...
        $(elem.currentTarget.previousElementSibling).val(0);
        $(elem.currentTarget).prop("disabled", true).css("opacity","0.25");
...
const totalCart = () => {
    ...
    panierArray.forEach(item => {
        totalCart += catalog[item.id].price * item.quantity
    });

    $('#total')[0].textContent = totalCart;
